...
export function useTrackVolume(
  ...
  React.useEffect(() => {
    ...
    const { cleanup, analyser } = createAudioAnalyser(track, denoiseMethod, options);
...
export function useMultibandTrackVolume(
  ...
  React.useEffect(() => {
    ...
    const { analyser, cleanup } = createAudioAnalyser(track, denoiseMethod, opts.analyserOptions);
  ...
  }, [track, track?.mediaStream, JSON.stringify(options), denoiseMethod]);
